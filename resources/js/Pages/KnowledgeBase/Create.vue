    ...
    <div class="bg-white rounded-md shadow overflow-hidden max-w-full">
      <form @submit.prevent="store">
        <div class="p-8 -mr-6 -mb-8 flex flex-wrap">
          <text-input v-model="form.title" :error="form.errors.title" class="pr-6 pb-8 w-full lg:w-1/2" label="Judul" />

            <select-input v-model="form.type_id" :error="form.errors.type_id" class="pr-6 pb-8 w-full lg:w-1/3" :label="__('Tipe')" :required="true">
        ...
        <div class="px-8 py-4 bg-gray-50 border-t border-gray-100 flex justify-end items-center">
          <loading-button :loading="form.processing" class="btn-indigo" type="submit">Buat Informasi Dasar</loading-button>
        </div>
      </form>
    </div>
  </div>
...
export default {
  metaInfo: { title: 'Buat Informasi Dasar' },
  components: {
      LoadingButton,
      SelectInput,
      TextareaInput,
