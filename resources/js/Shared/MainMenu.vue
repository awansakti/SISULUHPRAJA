  ...
  components: {
    ...
    data(){
      return{
          user: null,
          menu_items: [
              {'name': 'Beranda', 'route': 'dashboard', 'url': 'dashboard', 'icon': 'dashboard'},
          ],
          enable_option : {}
      }
    },
    ...
    created() {
      ...
      const user_access = this.user.access
        ...
        if(enable_option.faq && (user_access.faq.read || user_access.faq.update || user_access.faq.create || user_access.faq.delete)){
            this.menu_items.push({'name': 'Forum Tanya Jawab', 'route': 'faqs', 'url': 'faqs', 'icon': 'faq'})
        }
        ...
        if(enable_option.kb && (user_access.knowledge_base.read || user_access.knowledge_base.update || user_access.knowledge_base.create || user_access.knowledge_base.delete)){
            this.menu_items.push({'name': 'Informasi Dasar', 'route': 'knowledge_base', 'url': 'knowledge_base', 'icon': 'knowledge'})
        }

        if(user_access.customer.read || user_access.customer.update || user_access.customer.create || user_access.customer.delete){
            this.menu_items.push({'name': 'Daftar Pengguna', 'route': 'customers', 'url': 'customers', 'icon': 'all_users'})
        }
        ...
        if(user_access.user.read || user_access.user.update || user_access.user.create || user_access.user.delete){
            this.menu_items.push({'name': 'Pengelolaan Pengguna', 'route': 'users', 'url': 'users', 'icon': 'users'})
        }

        const DataMasterSubmenus = [];
        if(user_access.global.read || user_access.global.update || user_access.global.create || user_access.global.delete){
            DataMasterSubmenus.push({'name': 'Satker', 'route': 'global', 'url': 'settings/global', 'icon': 'global_setting'})
        }
        if(user_access.global.read || user_access.global.update || user_access.global.create || user_access.global.delete){
            DataMasterSubmenus.push({'name': 'Kalurahan', 'route': 'global', 'url': 'settings/global', 'icon': 'global_setting'})
        }
        if(user_access.global.read || user_access.global.update || user_access.global.create || user_access.global.delete){
            DataMasterSubmenus.push({'name': 'Laporan', 'route': 'global', 'url': 'settings/global', 'icon': 'global_setting'})
        }

        if(DataMasterSubmenus.length){
            this.menu_items.push({'name': 'Data Master', 'route': '', 'url': 'settings', 'icon': 'types', 'submenu': DataMasterSubmenus })
        }
        ...
        if(this.user.role.slug === 'admin'){
            settingSubmenus.push({'name': 'Laporan', 'route': 'settings.piping', 'url': 'settings/piping', 'icon': 'ticket'})
        }

        if(settingSubmenus.length){
            this.menu_items.push({'name': 'Pengaturan', 'route': '', 'url': 'settings', 'icon': 'settings', 'submenu': settingSubmenus })
        }
